...
int ngx_daemon() {
	...
	case 0:
		...
		//下面只有子进程才能执行
		if(setsid() == -1) {  //创建会话，脱离终端，终端和子进程无关
			printf("创建会话失败\n");
			return -1;
		}

		umask(0); //设置为 0，不要让它来限制文件权限，避免引流混乱
		...
		//动他之前，先close]，类似于指针指向null，让/dev/null成为标准输入；
		if( dup2(fd,STDIN_FILENO ) == -1)
		{
			printf(" dup2(fd,STDIN_FILENO ) 失败\n");
			return -1;
		}
		// /先关闭STDIN_FILENO，类似于指针指向null，让/dev/null成为标准输出；
		if( dup2(fd,STDOUT_FILENO) == -1 )
		{
			printf("dup2(fd,STDOUT_FILENO) 失败\n");
			return -1;
		}
...
```bash
 ps -eo pid,ppid,tty,pgrp,comm,stat,cmd | grep -E 'bash|PID|nginx'
