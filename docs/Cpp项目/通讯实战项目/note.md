...
- LT 是水平触发，属于低速模式，如果这个事件没处理完，就会被一直触发

- ET 是边缘触发，属于高速模式，这个事件的通知只会出现一次

在项目中，当一个新客户端通过三次握手接入服务器后，会执行`CSocket::ngx_event_accpet`函数，这个函数中使用了`CSocket::ngx_epoll_add_event`为接入的 socket 增加读事件到 epoll 的红黑树中，并设置了边缘触发模式（`EPOLLET`）,同时，设置收到客户端数据时的读处理函数为`CSocket::ngx_wait_request_hendler` （客户端在新连接上发送一些字符串，服务器就会触发这个函数）

一般 ET效率更高，不过 ET 模式编程更加麻烦，因为收到数据时，要不停地调用 revv 收取，一直收到 recv 返回 -1 并且 errno 值等于 EAGAIN 为止。

LT 模式的特点是一旦有数据没收完，系统会不断通知程序来收数据。

> 比如，如果一个连接发送来了数据，一个读事件就会被内核放进双向链表，如果我们不适用 recv 来接受数据，或者只是用 recv 接受了部分数据（比如发送来了 100 字节，只接受了 30 字节，还剩 70 字节，也就是针对这个 TCP 连接的接受缓冲区中有数据没读完），在 LT 模式下，内核就不会把该读事件的节点从双向链表中删除，这样每次程序调用 `epoll_wait` 函数都能获取这个通知。
> 
> ET 模式就不一样，不管程序是否调用 recv 来接受数据，一旦从双向链表中把该读事件对应的节点取走，内核肯定把这个节点从双向链表中删除了，所以下次用 epoll_wait 去取事件是取不到的，除非后续客户端又发来了数据，内核会再次向这个双向链表中添加一个读事件的节点，程序使用epoll_wait 才能再次收到该读事件。
> 
> 所以 LT 模式下的双向链表中的表项（节点数）比 ET 模式下双向链表中的表项多，从这个角度来讲，LT 模式的效率可能稍微差一点。

本项目中，一开始的时候只考虑了数据包有固定模式，所以采用了 LT 模式，因为编程简单，清晰，写好了效率上估计也不差。把`CSocket::ngx_epoll_add_event` 函数的第 4 个参数保持为 0 ，就表示采用 LT 模式。

后来考虑到浏览器访问一个 Web 服务器页面时，发送的数据就可能没有固定格式，浏览器一次可能向 Web 服务器发送一大批数据，然后等 Web 服务器回应。所以如果收发数据包没有固定格式的情况下，就采用 ET 模式，反复收数据，收完为止。
