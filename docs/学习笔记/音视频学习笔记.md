...
> 之前在[简单捋一捋ffplay源码中音视频播放的流程](https://mp.weixin.qq.com/s?__biz=MzkwMjIzNjc4NA==&mid=2247483951&idx=1&sn=fba735817397bc9aaa6be09f0d82bdac&chksm=c0a9dd30f7de5426aa81a6eb808873831366e8442a0f818e13aafdf13d1009668a89ccf40a12#rd)介绍的播放器的基本原理总说到，由于播放的整个流程中由于存在多线程的情况，所以会用到很多线程安全队列，其中最主要的就是  `PacketQueue` 和 `FrameQueue`。这一节将着重分析 `PacketQueue`

ffplay 用 `PacketQueue` 保存解封装后的数据，也就是保存着 AVPacket （关于 AVPacket 后面会在后面介绍 ffmepg 的数据结构中详细介绍），在 ffplay 的源码中定义了⼀个结构体 `MyAVPacketList` 
...
其实这个数据结构可以简单理解为就是队列的一个节点，可以通过 next 访问下一个节点。

serial 字段主要用于标记当前节点的序列号，ffplay 中很多地方都会用到 serial 这个概念，一般用于区分是否连续数据。

接着定义另⼀个结构体 PacketQueue

```cpp
typedef struct PacketQueue {
MyAVPacketList *first_pkt, *last_pkt; // 队⾸，队尾指针
    int nb_packets;       // 包的数量，也就是队列元素数量
    int size;              // 队列所有元素的数据⼤⼩总和
    int64_t duration;    // 队列所有元素的数据播放持续时间
    int abort_request;  // ⽤户退出请求标志
    int serial;         // 播放序列号，和MyAVPacketList的serial作⽤相同
    SDL_mutex *mutex;   // ⽤于维持PacketQueue的多线程安全( SDL_mutex 可以按 pthread_mutex_t 理解）
    SDL_cond *cond;     // ⽤于读、写线程相互通知( SDL_cond 可以按 pthread_cond_t 理解)
} PacketQueue;
```

这个结构体内定义了“队列”⾃身的属性。上⾯的注释对每个字段作了简单的介绍，这⾥也看到了 serial 字段，
MyAVPacketList 的 serial 字段的赋值来⾃ PacketQueue 的 serial，每个 PacketQueue 的 serial 是独⽴的。

> ⾳频、视频、字幕流都有⾃⼰独⽴的 PacketQueue。
...
初始化⽤于初始各个字段的值，并创建 mutex 和 cond：

```cpp
static int packet_queue_init(PacketQueue *q)
{
    memset(q, 0, sizeof(PacketQueue));
    q->mutex = SDL_CreateMutex();  //创建 mutex
    if (!q->mutex) {
        av_log(NULL, AV_LOG_FATAL, "SDL_CreateMutex(): %s\n", SDL_GetError());
        return AVERROR(ENOMEM);
    }
    q->cond = SDL_CreateCond();  //创建 cond
    if (!q->cond) {
...
相应的，`packet_queue_destroy()` 销毁过程负责清理 mutex 和 cond:
...
`flush_pkt` 定义是 static AVPacket flush_pkt; ，是⼀个特殊的 packet，主要⽤来作为⾮连续的两端数据的“分界”标记：
- 插⼊ flush_pkt 触发 PacketQueue 其对应的 serial，加 1 操作
...
这⾥ SDL_CondSignal 的作⽤在于确保当前等待该条件的线程能被激活并继续执⾏退出流程，并唤醒者会检测 abort_reques t标志确定⾃⼰的退出流程。
...
主要实现在函数 packet_queue_put_private，这里需要注意的是如果放入失败，需要释放 AVPacket。
...
对于 packet_queue_put_private 主要完成 3 件事：

- 计算serial。serial标记了这个节点内的数据是何时的。⼀般情况下新增节点与上⼀个节点的serial是⼀样的，但当队列中加⼊⼀个 flush_pkt 后，后续节点的 serial 会⽐之前⼤ 1，⽤来区别不同播放序列的
packet.
- 节点⼊队列操作。
- 队列属性操作。更新队列中节点的数⽬、占⽤字节数（含 AVPacket.data 的⼤⼩）及其时⻓。主要⽤来控制Packet队列的⼤⼩，我们 PacketQueue 链表式的队列，在内存充⾜的条件下我们可以⽆限 put ⼊ packet，如果我们要控制队列⼤⼩，则需要通过其变量 size、duration、nb_packet s三者单⼀或者综合去约束队列的节点的数量，具体在 read_thread 进⾏分析。
    ...
    for (;;) {
        ...
        if (pkt1) { //队列中有数据
            ...
            if (!q->first_pkt)
                q->last_pkt = NULL;
            q->nb_packets--; //节点数减1
            q->size -= pkt1->pkt.size + sizeof(*pkt1); //cache⼤⼩扣除⼀个节点
            q->duration -= pkt1->pkt.duration; //总时⻓扣除⼀个节点
            //返回AVPacket，这⾥发⽣⼀次AVPacket结构体拷⻉，AVPacket的data只拷⻉了指针
            *pkt = pkt1->pkt;
            if (serial) //如果需要输出serial，把serial输出
                *serial = pkt1->serial;
            av_free(pkt1); //释放节点内存,只是释放节点，⽽不是释放AVPacket
...
该函数整体流程：
- 加锁
- 进⼊ for 循环，如果需要退出 for 循环，则 break；当没有数据可读且 block 为 1 时则等待
  - ret = -1 终⽌获取 packet
  - ret = 0 没有读取到 packet
  - ret = 1 获取到了 packet
- 释放锁
...
1. 队列操作：出队列操作； nb_packets 相应 -1; duration 的也相应减少， size 也相应占⽤的字节⼤⼩（`pkt1->pkt.size + sizeof(*pkt1)`）
2. 给输出参数赋值：就是 MyAVPacketList 的成员传递给输出参数 pkt 和 serial
3. 释放节点内存：释放放⼊队列时申请的节点内存（注意是节点内存⽽不是 AVPacket 的数据的内存）

### packet_queue_put_nullpacket()

放⼊“空包”（ nullpacket ）。放⼊空包意味着流的结束，⼀般在媒体数据读取完成的时候放⼊空包。放⼊空包，⽬的是为了冲刷解码器，将编码器⾥⾯所有 frame 都读取出来
...
packet_queue_flush ⽤于将 packet 队列中的所有节点清除，包括节点对应的 AVPacket。⽐如⽤于退出播放和 seek 播放

- 退出播放，则要清空 packet queue 的节点
- seek 播放，要清空 seek 之前缓存的节点数据，以便插⼊新节点数据
...
函数主体的 for 循环是队列遍历，遍历过程释放节点和 AVPacket 。最后将 PacketQueue 的属性恢复为空队列状态。

到目前为止，我们分析了 PacketQueue 的实现和主要的操作方法。

### PacketQueue总结

前⾯我们分析了 PacketQueue 的实现和主要的操作⽅法，现在总结下两个关键的点：

#### 第⼀，PacketQueue 的内存管理

![](https://cdn.jsdelivr.net/gh/kendall-cpp/blogPic@main/blog-img-01/packetQueue01.2ufhr4pq3920.webp)

MyAVPacketList的内存是完全由PacketQueue维护的，在put的时候malloc，在get的时候free。
AVPacket分两块：
- ⼀部分是AVPacket结构体的内存，这部分从 MyAVPacketList 的定义可以看出是和 MyAVPacketList 共存亡的。
- 另⼀部分是AVPacket字段指向的内存，这部分⼀般通过 av_packet_unref 函数释放。⼀般情况下，是在 ge t后由调⽤者负责⽤ av_packet_unref 函数释放。特殊的情况是当碰到 packet_queue_flush 或 put 失败时，这时需要队列⾃⼰处理。

#### 第⼆，serial的变化过程

![](https://cdn.jsdelivr.net/gh/kendall-cpp/blogPic@main/blog-img-01/packetQueue02.2hvu71mepjq0.webp)


如上图所示，左边是队头，右边是队尾，从左往右标注了 4 个节点的 serial ，以及放⼊对应节点时 queue 的 
serial 。

可以看到放⼊ flush_pkt 的时候后，serial 增加了1.假设，现在要从队头取出⼀个节点，那么取出的节点是 serial 1，⽽ PacketQueue ⾃身的 queue 已经增⻓到了 2。

### PacketQueue 设计思路

- 设计一个多线程安全的队列，保存 AVPacket ，同时统计队列内已缓存的数据大小。（这个统计数据会用来后续设置要缓存的数据量）

- 引入 serial 的概念，区别前后数据包是否连续

- 设计了两类特殊的 packet: flush_pkt 和 nullpkt ，用于更细致的控制（类似用于多线程编程的事件模型: 往队列中放入 flush 事件、放入 null 事件。）

> 下一篇将会介绍 `FrameQueue`，小伙伴们点击下面链接关注起来吧。
