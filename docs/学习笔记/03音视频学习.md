...
> 在录音结束的时候窗口点击不了，也无法关闭，所以使用子线程来录音，窗口依然可以正常控制
...
使用 SDL 多媒体开发库 播放 pcm

使用命令行播放

```
ffplay.exe -ar 44100 -ac 2 -f s16le in.pcm
```
...
pcm 文件填充到音频缓冲区，stream 指向音频缓冲区，音频缓冲区的大小就是靠 sample 来决定的，放 1024 个样本，每个样本都是 16*2 =32 位 = 4 字节（双声道） 位的。所以音频缓冲区大小是 4096 个字节

所以缓冲区的大小：希望填充的大小 `(samples * format * channels / 8)`
