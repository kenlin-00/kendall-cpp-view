...
public:
    ...
    void backtracking(TreeNode *root) {
        ...
        if(root->left) {
            binaryTreePaths(root->left);  //注意这里不是 backtracking ，而是又当做一棵树
            path.pop_back();  //回溯
        }
        if(root->right) {
...
在中序遍历过程中直接计算，需要记录前一个节点的值，也可以直接记录前一个节点
...
[参考题解](https://programmercarl.com/0112.%E8%B7%AF%E5%BE%84%E6%80%BB%E5%92%8C.html)
