...
public:
    ...
    int pop() {
        ...
        while(queDate.size() > 1) {
            queHelp.push(queDate.front());
            queDate.pop();
        }
        ans = queDate.front();
        queDate.pop();
        while(!queHelp.empty()) {
            queDate.push(queHelp.front());
            queHelp.pop();
        }
        return ans;
...
int main() {
    MyQueue *que = new MyQueue();  //不能写成 MyQueue *que;
    cout << que->empty() << endl;  //为空
    que->push(1);
    que->push(2);
    que->push(3);
    cout << que->empty() << endl;  //不为空
    cout << que->peek() << endl;
    que->pop();
    cout << que->peek() << endl;
...
【要求】 Insert、delete 和  getRandom方法的时间复杂度都是 O(1)。

使用两个哈希表，map1 的 key 对应 map2 的 value，map2 的 key 对应 map1 的 value，

在没有 delete 操作的情况下，key 是连续的，但是如果有 delete 操作，那么就不是连续的。

删除了中间的，就从 map2 中找到最后一个元素插入 map1 删除的位置，然后等概率返回 key，这样就可以保证取每个元素的概率是相等的了。
...
int main() {
    ...
    head->next->next->next->next = new ListNode(5);
    head->next->next->next->next->next = new ListNode(5);
    head->next->next->next->next->next->next = head->next->next->next;  //换入口8

    ListNode *ans = EntryNodeOfLoop(head);
    if(ans == nullptr) {
        cout << "no cycle" << endl;
    }
    else 
...
[leetcode题目](https://leetcode-cn.com/problems/palindrome-number/submissions/)
...
// 在判断最后一个数字 3 的时候 help = 1
n = (n % help) / 10 = (3 % 1) / 10 = 3 / 10 = 0
// n = 0 就是循环终止条件
```
