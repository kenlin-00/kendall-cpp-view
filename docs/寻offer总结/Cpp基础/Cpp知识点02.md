...
* **多态**：是“一个接口，多个实现”，通过父类调用子类的成员，实现了接口重用。如父类的指针指向子类的对象。**C++中多态的实现机制是虚函数**==>[多态的实现](/寻offer总结/Cpp基础/Cpp知识点02?id=多态的实现) ==>[C++虚函数相关](/寻offer总结/Cpp基础/Cpp知识点02?id=c虚函数相关，虚函数的实现原理（热门，重要）)
