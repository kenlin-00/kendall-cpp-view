...
每连入一个客户端，m_timerQueuemap「定时器队列」 这个时间队列中就增加一项，这一项实际是用来监视这个用户是否到达一定时间迟迟不向客户端发心跳包。如果这一项过期了，就从时间队列中删除。



------

- 服务器按照**包头包体**格式正确的接受客户端发送的数据包
- 根据收到的包的不同来执行不同的业务处理逻辑
- 把业务处理产生的结果数据包返回给客户端

使用到的技术： epoll，线程池技术，线程之间的同步技术


框架

- 一个 master 进程，多个 worker 子进程的进程框架

- nginx 热更新，worker子进程挂了 master能够重新启动 worker 进程，重载配置文件（没讲）

- nginx 使用的是 ET 边缘触发模式，本项目使用的是水平出发模试 LT，来实现网络通讯数据的接收。

- 参考 nginx 实现接受数据包和发送数据包
- 自己实现线程池，把客户端发送过来的数据放到线程池，然后触发线程去干活，直到处理完了将数据送回客户端。
- 实现一套连接池，连接池中的 延迟 回收，以及专门处理数据发送的发送线程。
