...
内存池释放需要走 ngx_destroy_pool，但是独立大内存块的单独释放，可以走 ngx_pfree 方法
...
> 减少 malloc / new 的使用频率，提高内存分配效率，方式内存管理混乱和内存碎片的产生          
> Nginx 中内存池分成两部分：大内存和小内存      
> 设置一个 阈值，如果需要申请的内存     
> 小内存逻辑：循环读取内存池中的内存块，不够的话就需要申请新的内存块            
> 大内存逻辑：直接调用 `ngx_palloc_large ` 方法申请一块独立的内存块，并且将内存块挂载到 `pool->large` 的链表上进行统一管理。        
