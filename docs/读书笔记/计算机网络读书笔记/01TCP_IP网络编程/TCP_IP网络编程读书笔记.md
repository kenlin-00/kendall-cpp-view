...
一连接电话线，电话机就可以转换为**可接听状态**，这时其他人可以拨打电话请求连接到该机。同样，需要把套接字转化成可接受连接状态。
...
2. 第二步：调用 bind 函数分配(绑定)IP地址和端口号。
3. 第三步：调用 listen 函数转换为**可接受请求**状态。
...
服务器端代码请参见：[hello_server.c](读书笔记/计算机网络读书笔记/01TCP_IP网络编程/ch01/code.md?id=hello_serverc)

**备注**：

`struct sockaddr`和`struct sockaddr_in`这两个结构体用来处理网络通信的地址。
 - sockaddr      
 `sockaddr`在头文件`#include <sys/socket.h>`中定义，`sockaddr`的缺陷是：`sa_data`把目标地址和端口信息混在一起了
...
客户端代码请参见：[hello_client.c](读书笔记/计算机网络读书笔记/01TCP_IP网络编程/ch01/code.md?id=hello_client)
...
运行的时候，首先再 9190 端口启动服务，然后 heserver 就会一直等待客户端进行响应，当客户端监听位于本地的 IP 为 127.0.0.1 的地址的9190端口时，客户端就会收到服务端的回应，输出`Message from server : hello world! `
...
代码见：[low_open.c](读书笔记/计算机网络读书笔记/01TCP_IP网络编程/ch01/code.md?id=low_open)
